...
- **TS baseline = 139** via `npx tsc --noEmit 2>&1 | grep -c "error TS"`. Split: **16** in
  ...
  a baseline looks stubbornly high: check what is DEAD before assuming it is load-bearing.
  Then 132 -> 134 the same day: two new `Deno.env.get` lines in loops-sync and superwall-webhook,
  one `Cannot find name 'Deno'` each. App-code stayed 27 throughout — watch that number.) (Was 149/104 until 2026-08-29: adding one
...
- **Unit tests run under plain node, no Deno CLI and no jest:**
  `node --test lib/*.test.ts supabase/functions/_shared/*.test.ts` — **611 tests** as of 2026-09-14 (138 on 2026-08-29).
