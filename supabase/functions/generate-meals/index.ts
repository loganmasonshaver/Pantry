...
- USE INGREDIENTS IN THEIR CORRECT FORM AND STATE (mandatory): the pantry names each item's specific form — respect it, and match the dish, the cooking steps, and prepTime to that form. Never silently swap to a different form. If the on-hand form doesn't fit a dish, either use it correctly or pick a dish where it IS authentic.
  ...
  • NAME THE PREPARED FORM, NOT THE PANTRY ITEM (blocking). When a pantry item has to be transformed before anyone can eat it, the ingredient NAME must be what actually goes into the dish: coffee beans → "brewed coffee" or "espresso"; dry pasta cooked in the steps → "cooked pasta"; uncooked rice → "cooked rice"; whole spices you grind → "ground <spice>". The ingredient name is BOTH the line the cook reads AND the description the dish photo is generated from, so writing "coffee beans" for a shot of espresso puts a pile of whole roasted beans on top of the finished oatmeal. If a step says "brewed coffee", the ingredient must say brewed coffee too. KNIFE WORK IS A PREPARED FORM TOO: if a step uses something diced, minced, sliced, chopped, grated, shredded or cubed, that word belongs in the ingredient NAME ("diced yellow onion" with visual "1/4 medium") — the cook preps from the ingredient list, so a cut whose first mention is inside a cooking step ("add diced onions" under an onion line that reads "1/4 medium") is a cut nobody was told to make. If the cut would misdescribe the pantry item, give it its own step before the step that uses it.
