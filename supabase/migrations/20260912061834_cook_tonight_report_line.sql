-- Cook Tonight quality in the daily report: one line under the Discover health line, red when a
-- sweep-derived threshold is crossed. Reads the funnel every real generation already writes to
-- pipeline_runs (provider generate-meals-funnel, dry_run = false), so a prompt regression after
-- launch shows up the next morning. Counts only; never a user.
--
-- Full CREATE OR REPLACE of ops_report_data() — the body is otherwise identical to
-- 20260910214408_dislike_flavours.sql.
...
begin
  ...
  select (select count(*) from ok),
    ...
    into v_ct_gens, v_ct_failed, v_ct_meals, v_ct_unseasoned, v_ct_floor, v_ct_nck,
         v_ct_diet, v_ct_clash, v_ct_axes, v_ct_short_decks, v_ct_repeats;
  ...
  v_ct_bad := v_ct_failed > 0 or v_ct_clash > 0
