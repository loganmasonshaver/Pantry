-- Daily report: Cook Tonight goes red when any savory meal was shown with no flavour axis.
-- generate-meals now records funnel.savoryZeroAxesShown (savory meals in the deck that reach none
-- of acid / heat / umami / aromatic-in-fat; sweet dishes exempt). The previous red lines checked
-- only salt and pepper, so run 678's deck — three meals, zero axes, a soup built on water — read
-- as healthy. Full CREATE OR REPLACE of ops_report_data(); the body is otherwise identical to
-- 20260912061834_cook_tonight_report_line.sql.
...
declare
  ...
  v_html text;
  v_text text;
  -- Cook Tonight, from the funnel every real generation writes (pipeline_runs). Aggregates only.
  v_ct text;
  v_ct_bad boolean;
  v_ct_gens int; v_ct_failed int; v_ct_meals int; v_ct_unseasoned int; v_ct_floor int; v_ct_nck int;
  v_ct_diet int; v_ct_clash int; v_ct_axes numeric; v_ct_short_decks int; v_ct_repeats int;
  v_ct_zero int;
...
begin
  ...
  -- Cook Tonight quality over the last 24h. Every number here is already recorded per generation
  -- by generate-meals (the same counters the sweep harness scores), so this is aggregation only.
  -- A failed generation writes a row with funnel.failed = true, so "generations" is honest.
  with g as (
    select stored, funnel
      from pipeline_runs
     where provider = 'generate-meals-funnel' and dry_run = false
       and created_at >= now() - interval '24 hours'
  ),
  ok as (select * from g where not coalesce((funnel->>'failed')::boolean, false)),
  steps as (
    select (s->>'unseasoned')::boolean as unseasoned
      from ok, jsonb_array_elements(coalesce(ok.funnel->'stepIssuesShown', '[]'::jsonb)) s
  ),
  axes as (
    select a::int as n from ok, jsonb_array_elements_text(coalesce(ok.funnel->'flavourAxesShown', '[]'::jsonb)) a
  )
  select (select count(*) from ok),
         (select count(*) from g where coalesce((funnel->>'failed')::boolean, false)),
         (select coalesce(sum(stored), 0) from ok),
         (select count(*) filter (where unseasoned) from steps),
         (select coalesce(sum((funnel->>'belowProteinFloorShown')::int), 0) from ok),
         (select coalesce(sum((funnel->>'notCookableKept')::int), 0) from ok),
         (select coalesce(sum((funnel->>'droppedByDiet')::int), 0) from ok),
         (select coalesce(sum((funnel->>'savoryClashShown')::int), 0) from ok),
         (select round(avg(n), 1) from axes),
         (select count(*) from ok where stored > 0 and (funnel->>'belowProteinFloorShown')::int >= stored),
         (select coalesce(sum((funnel->>'repeatsShown')::int), 0) from ok),
         (select coalesce(sum((funnel->>'savoryZeroAxesShown')::int), 0) from ok)
    into v_ct_gens, v_ct_failed, v_ct_meals, v_ct_unseasoned, v_ct_floor, v_ct_nck,
         v_ct_diet, v_ct_clash, v_ct_axes, v_ct_short_decks, v_ct_repeats, v_ct_zero;

  -- Red lines, from the sweep baselines: unseasoned was 50% before the seasoning rule and ~20%
  -- after; uncookable-shown is the accepted ~3%; any savory clash shown means the gate broke.
  -- Any SAVORY meal shown with no flavour axis is red: the prompt demands two, and a salted,
  -- flavourless deck read as healthy for a week because only salt was checked (run 678).
  v_ct_bad := v_ct_failed > 0 or v_ct_clash > 0 or v_ct_zero > 0
    or (v_ct_meals > 0 and (v_ct_unseasoned::numeric / v_ct_meals > 0.30
                            or v_ct_nck::numeric / v_ct_meals > 0.10
                            or v_ct_floor::numeric / v_ct_meals > 0.20));
  v_ct := case
    when v_ct_gens = 0 and v_ct_failed = 0 then 'Cook Tonight: no generations'
    else 'Cook Tonight: ' || v_ct_gens || case when v_ct_gens = 1 then ' generation' else ' generations' end
      || case when v_ct_failed > 0 then ' (' || v_ct_failed || ' FAILED)' else '' end
      || ' · ' || v_ct_meals || ' meals'
      || ' · ' || v_ct_floor || ' under protein floor'
      || ' · ' || v_ct_nck || ' uncookable shown'
      || ' · ' || v_ct_diet || ' diet drops'
      || ' · ' || case when v_ct_meals > 0 then round(100.0 * v_ct_unseasoned / v_ct_meals) else 0 end || '% unseasoned'
      || ' · avg ' || coalesce(v_ct_axes::text, '0') || ' flavour axes'
      || case when v_ct_zero > 0 then ' · ' || v_ct_zero || ' SAVORY WITH NO FLAVOUR AXIS' else '' end
      || ' · ' || v_ct_repeats || ' repeats'
      || case when v_ct_short_decks > 0 then ' · ' || v_ct_short_decks || case when v_ct_short_decks = 1 then ' deck' else ' decks' end || ' entirely under floor (pantry warning shown)' else '' end
  end;
  ...
  v_html := '<div style="font-family:-apple-system,BlinkMacSystemFont,Helvetica,Arial,sans-serif;'
    ...
    || '<div style="font-size:14px;margin-top:4px;' || case when v_ct_bad then 'color:#e5484d;font-weight:700' else 'color:' || c_grey end
    || '">' || public.ops_html(v_ct) || '</div>'
