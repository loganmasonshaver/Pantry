...
// Post-scan payoff: reveals the cook-now meals generated from the just-scanned pantry as a card deck.
// The choreography is built around one engineered PEAK — the hero card landing with a glow bloom +
// success haptic — because that single moment is what the user remembers.
//
// A component, not a screen, because the scan modal renders it as its own last step. Pushed as a
...
export function CookRevealView({ onClose, onOpenMeal, edges = ['top', 'bottom'], imagesWaitMs = IMAGES_WAIT_MS }: {
  ...
  return (
    <SafeAreaView style={styles.safe} edges={edges}>
      {/* Close → back to the screen the scan was launched from */}
      ...
      {error && revealed.length === 0 ? (
        ...
        <View style={styles.body}>
            {/* Same headline instance as the build-up — it already assembled, so it just persists
                here rather than replaying. Nothing under it: the message is the headline, and the
                photos get the room (Logan). What the scan was built around is told in big type on
                the scan wait (lib/scanStory); each card's pills carry that meal's own numbers. */}
            <View style={styles.header}>
              <Text style={styles.eyebrow}>FROM YOUR PANTRY</Text>
              <HeadlineChunks count={revealed.length} anims={chunkAnims} />
            </View>
