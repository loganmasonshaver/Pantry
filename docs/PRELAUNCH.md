...
- [ ] **PLANNED, GATED ON LOGAN'S "GO" (2026-09-15) — screen transitions + a small animation and
  ...
  - [ ] Phase 2 — **AUDIT DONE, PROPOSAL AWAITING LOGAN'S OK** (PLAN-motion §4 "Phase 2 audit"): add haptics to ~20 commits that have none (Log a food, pantry toggle, grocery check, Profile has zero), move Save's and Extract Recipe's haptic to after the result, remove haptics from openers and non-touch events, give New picks press feedback, fade in user-added Grocery rows and meal slots. Tab-bar tick left as Logan's call. Build only after Phase 1 is measured, so the two phases' numbers stay separable.
