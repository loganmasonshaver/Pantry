...
const w1 = StyleSheet.create({
  trailerWrap: { flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 24 },
    ...
    const recipes: Record<string, Record<string, Recipe[]>> = {
      Classic: {
        ...
        Lunch: [
          ...
          { name: 'Seared Ahi Tuna Salad', Icon: Fish, tint: TEAL, contains: ['Fish', 'Sesame'], skill: 'medium', prepMin: 15 },
        ...
        Dinner: [
          ...
          { name: 'Grilled Sirloin Steak with Asparagus', Icon: Drumstick, tint: '#60A5FA', contains: ['Beef'], skill: 'medium', prepMin: 20 },
          { name: 'Herb-Crusted Salmon with Green Beans', Icon: Fish, tint: '#60A5FA', contains: ['Fish'], skill: 'medium', prepMin: 25 },
          { name: 'Garlic Butter Shrimp over Cauliflower Rice', Icon: Drumstick, tint: '#60A5FA', contains: ['Shellfish', 'Dairy'], skill: 'medium', prepMin: 20 },
          { name: 'Lamb Chops with Roasted Root Vegetables', Icon: Drumstick, tint: '#60A5FA', contains: [], skill: 'medium', prepMin: 30 },
          { name: 'Pan-Seared Pork Tenderloin with Asparagus', Icon: Drumstick, tint: '#60A5FA', contains: [], skill: 'medium', prepMin: 25 },
        ...
        'Main Meal': [
          { name: 'Grilled Chicken Rice Bowl', Icon: Drumstick, tint: TEAL, contains: [], skill: 'easy', prepMin: 20 },
          { name: 'Pan-Seared Chicken with Roasted Veggies', Icon: Drumstick, tint: '#60A5FA', contains: [], skill: 'easy', prepMin: 20 },
          { name: 'Lamb Chops with Roasted Root Vegetables', Icon: Drumstick, tint: TEAL, contains: [], skill: 'medium', prepMin: 30 },
          { name: 'Pan-Seared Pork Tenderloin with Asparagus', Icon: Drumstick, tint: TEAL, contains: [], skill: 'medium', prepMin: 25 },
          { name: 'Herb-Crusted Salmon with Green Beans', Icon: Fish, tint: TEAL, contains: ['Fish'], skill: 'medium', prepMin: 25 },
        ],
      },
      Pescatarian: {
        Breakfast: [
      ...
      if (user) {
        ...
        try {
          ...
          if (raw) {
            ...
            // Look up shared server-side image_cache for any plan meals whose images
            // were already generated by previous users. Without this, new users see
            // blank placeholder thumbnails on Home until they tap each meal individually
            // (which triggers per-meal auto-generation). Single batched query so we
            // don't N+1 the inserts.
            const normalizeKey = (name: string) =>
              name.toLowerCase().replace(/[^a-z0-9 ]/g, '').replace(/\s+/g, ' ').trim()
            const mealKeys = planMeals.map((m: any) => normalizeKey(m.name))
            const { data: cachedImages } = await supabase
              .from('image_cache')
              .select('meal_key, image_url')
              .in('meal_key', mealKeys)
            const sharedCache = new Map<string, string>(
              (cachedImages ?? []).map((r: any) => [r.meal_key, r.image_url])
            )
            ...
            await Promise.all(planMeals.map(m =>
              supabase.rpc('insert_saved_meal', {
                ...
                p_image_url:
                  m.image
                  ?? imgCache[m.name]
                  ?? sharedCache.get(normalizeKey(m.name))
                  ?? null,
              }).then(() => {}, () => {})
            ))
